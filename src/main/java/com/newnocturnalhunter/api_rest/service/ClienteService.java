 ...
 */
@Service
public class ClienteService implements UserDetailsService { // UserDetailsService para autenticación de usuarios
    @Autowired
    ...
    public ClienteRegisterDTO create(ClienteRegisterDTO clienteRegisterDTO) {
        ...
        cliente.setPassword(passwordEncoder.encode(clienteRegisterDTO.getPassword1())); // Encripta la contraseñas
    ...
    public List<ClienteDTO> getAllClientes() {
        ...
        List<ClienteDTO> clientesDTO = new ArrayList<>(); // Crea una lista vacia
        clientes.forEach(cliente -> clientesDTO.add(mapper.mapToClienteDTO(cliente))); // Recorre la lista de clientes y los agrega a la lista
        return clientesDTO;
    }
    ...
    public ClienteDTO update(String username, ClienteDTO clienteDTO) {
        ...
        // Busca el usuario en la base de datos y lo actualiza
        Cliente cliente = mapper.mapToCliente(clienteDTO);
        cliente.setUsername(clienteDTO.getUsername());
        cliente.setPassword(passwordEncoder.encode(clienteDTO.getPassword())); // Encripta la contraseñas
